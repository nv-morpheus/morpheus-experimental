...
def clean_nlp(text):
    text = clean(text)
    text = ' '.join([x for x in text.split(' ') if x.isnumeric()
                    == False and len(x) > 1 and len(x) < 21])
    return text
...
def create_features(df):
    ...
    df['subdomain_numbers_count'] = df['subdomain'].swifter.apply(
        get_count_numbers)
    df['subdomain_parts_count'] = df['subdomain'].swifter.apply(
        get_count_parts)
    ...
    df['brand_in_subdomain'] = df['subdomain'].swifter.apply(
        get_brand_in_subdomain)
    df['brand_in_path'] = df['path'].swifter.apply(get_brand_in_path)
    url_df['path_clean'] = url_df['path'].swifter.apply(lambda x: clean(x))
    url_df['path_max_len'] = url_df['path_clean'].swifter.apply(
        get_max_len_path)
...
def data_preprocessing(df):
    ...
    X_ben_train, X_ben_test, Y_ben_train, Y_ben_test = train_test_split(
        X_mal, Y_mal, train_size=0.25)  # train_test_split(
    # X_ben, Y_ben, train_size=0.8)
...
def train_model(X_train_nlp, X_train_features, Y_train):
    ...
    x = tf.keras.layers.Embedding(
        vocab_length, EMBEDDING_DIM, input_length=MAX_LEN)(inputA)
...
def model_eval(model, X_test_nlp, X_test_features, Y_test):
    ...
    for thr in np.arange(0, 1, 0.01):
        ...
        recall_step = len(X_test[(X_test['pred'] > thr) & (
            X_test['label'] == 1)]) / len(X_test[X_test['label'] == 1])
...
X_train_features, X_test_features, df_max_min = structural_processing(
    X_train, X_test)

print("Calculating NLP URL features...")
X_train_nlp, X_test_nlp, tokenizer_df, vocab_length = nlp_processing(
    X_train, X_test)

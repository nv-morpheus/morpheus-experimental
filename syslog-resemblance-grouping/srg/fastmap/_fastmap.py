...
class FastMap:
    ...
    def _candidate_distances(self, df, position, left_pivots):
        ...
        for i in range(self._num_models):
            pdf['model_%i' % i] = pdf.apply(lambda x: self._pivot_distance(left_pivots[i], x[self._col], position, i), axis=1, result_type='expand')
        return pdf

    def _groupby_candidate_distances(self, df, groupby, position, left_pivots):
        pdf = df.to_pandas().copy()
        for i in range(self._num_models):
            pdf['model_%i' % i] = pdf.apply(lambda x: self._group_pivot_distance(left_pivots[x[groupby]][i], x[self._col], position, i, x[groupby]), axis=1, result_type='expand')
        return pdf

    def fit(self, X, delimiter = None, names = None, npartitions: int = 2, column: Union[str, None] = None, groupby: Union[str, None] = None):
        if groupby is None:
    ...
    def _groupby_fit(self, X, delimiter, names, npartitions, column, groupby):
        ...
        try:
            ...
            try:
                ...
                print(e1)
                self._groups = None
        sample_fracs = {group: max(50, 2 * self._num_models) / group_count for group, group_count in ddf.groupby(groupby).count()[self._col].compute().to_pandas().to_dict().items()}
        left = ddf.head(1)[self._col].to_arrow().to_pylist()[0]
        self._pivots = {group: {model_num: {0: {'left': left, 'left_proj': [0.0 for _ in range(self._dim)], 'right': left, 'right_proj': [0.0 for _ in range(self._dim)], 'dist': 0.0}} for model_num in range(self._num_models)} for group in self._groups}
        for k in tqdm_notebook(range(self._dim), desc='Current dimension'):
            initial_group_samples = ddf.reduction(chunk=self._groupby_initial_pivots_chunks, chunk_kwargs={'groupby':groupby, 'sample_fracs':sample_fracs}, aggregate = self._groupby_initial_pivots_aggregate, meta=dict).compute()
            initial_pivots = {group:random.sample(list(samples), self._num_models) for group, samples in initial_group_samples.items()}
            ...
            for m in tqdm_notebook(range(self._iters), desc='Iteration', leave=False):
                ...
                assigned_ddf = ddf.map_partitions(lambda df: self._groupby_candidate_distances(df, groupby, k, left_pivots))
                right_pivots = assigned_ddf.reduction(chunk=self._groupby_chunk, chunk_kwargs={'groupby': groupby, 'lefts': left_pivots}, aggregate=self._groupby_aggregate, aggregate_kwargs={'lefts': left_pivots}, meta=dict).compute()
            for group in self._groups:
                for i in range(self._num_models):
                    left, right, dist = left_pivots[group][i], right_pivots[group][i][0], right_pivots[group][i][1]
                    final_pivots = {'left': left, 'left_proj': self._i_proj(left, k, i), 'right': right, 'right_proj': self._i_proj(right, k, i), 'dist': dist}
    ...
    def _full_fit(self, X, delimiter, names, npartitions, column):
        ...
        sample_frac = max(50, 3 * self._num_models) / n
        left = ddf.head(1)[self._col].to_arrow().to_pylist()[0]
        self._pivots = {model_num: {0: {'left': left, 'left_proj': [0.0 for _ in range(self._dim)], 'right': left, 'right_proj': [0.0 for _ in range(self._dim)], 'dist': 0.0}} for model_num in range(self._num_models)}
        for k in tqdm_notebook(range(self._dim), desc='Current dimension'):
            initial_pivots = random.sample(list(set(ddf[self._col].sample(frac=sample_frac).compute().to_arrow().to_pylist())), self._num_models)
            right_pivots = [[left, 0.0] for left in initial_pivots]
            for m in tqdm_notebook(range(self._iters), desc='Iteration', leave=False):
                ...
                right_pivots = assigned_ddf.reduction(chunk=self._chunk, chunk_kwargs={'lefts': left_pivots}, aggregate=self._aggregate, aggregate_kwargs={'lefts': left_pivots}, meta=list).compute()
            for i in range(self._num_models):
    ...
    def transform(self, X, model: int = None, delimiter = None, names = None, npartitions: int = 2, column: Union[str, None] = None, group: Union[str, None] = None):
    ...
    def _groupby_models_transform(self, X, model, delimiter, names, npartitions, column, group):
        ...
        #     ddf = dask_cudf.from_cudf(cdf, npartitions=npartitions)
            ...
            projs = [self._group_i_proj(X, self._dim, m, group) for m in range(self._num_models)]
            return projs[0] if self._num_models == 1 else projs
        assert isinstance(ddf, dask_cudf.core.DataFrame), 'X must be a path to a csv or a (dask-)cudf Series or DataFrame, or a projectable object'
    ...
    def _group_assign_single_projs(self, df, col, groupby, model):
        pdf = df.to_pandas().copy()
        x_cols = ['x_%i' % i for i in range(self._dim)]
        pdf[x_cols] = pdf.apply(lambda x: self._group_i_proj(x[col], self._dim, model, x[groupby]), axis=1, result_type='expand')
        return pdf

    def _group_assign_multi_projs(self, df, col, groupby, model):
        pdf = df.to_pandas().copy()
        for i in range(self._num_models):
            pdf['model_%i' % i] = pdf.apply(lambda x: self._group_i_proj(x[col], self._dim, i, x[groupby]), axis=1, result_type='reduce')
        return pdf

    def _compute_proj_i(self, index, model_num, i, obj, obj_proj):

        left_dist = self._dist(self._pivots[model_num][i]['left'], self._pivots[model_num][i]['left_proj'], obj, obj_proj, index)
        right_dist = self._dist(self._pivots[model_num][i]['right'], self._pivots[model_num][i]['right_proj'], obj, obj_proj, index)
    ...
    def _group_compute_proj_i(self, index, model_num, group, i, obj, obj_proj):
        ...
        left_dist = self._dist(self._pivots[group][model_num][i]['left'], self._pivots[group][model_num][i]['left_proj'], obj, obj_proj, index)
        right_dist = self._dist(self._pivots[group][model_num][i]['right'], self._pivots[group][model_num][i]['right_proj'], obj, obj_proj, index)
    ...
    def _groupby_chunk(self, df, groupby, lefts):
        ...
        for row in df.to_dict('records'):
            ...
            for model_num in range(self._num_models):
                incoming_dist = row['model_%i' % model_num]
                if (incoming_dist > candidates[incoming_group][model_num][1]) and (row[self._col] not in current_candidates[incoming_group]):
        ...
        for chunk in chunks:
            for group, candidate_distances in chunk.items():
                for model_num, chunk_candidates in enumerate(candidate_distances):
                    if (candidates[group][model_num][1] < chunk_candidates[1]) and (chunk_candidates[0] not in current_candidates[group]):
